	...
	tinymce.create('tinymce.Editor', {
		Editor : function(id, s) {
			...
			t.settings = s = extend({
				...
				font_size_style_values : '8px,9px,10px,11px,12px,13px,14px,15px,16px,17px,18px,19px,20px,21px,22px,23px,24px,25px,26px,27px,28px,29px,30px,31px,32px,33px,34px,35px,36px',//'xx-small,x-small,small,medium,large,x-large,xx-large',
		...
		init : function() {
			...
			if (s.popup_css !== false) {
				if (s.popup_css)
					...
					s.popup_css = t.baseURI.toAbsolute("themes/" + s.theme + "/assets/" + s.skin + "/dialog.css");
			}
			if (s.popup_css_add)
