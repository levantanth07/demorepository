<?php
class ListManageCommentForm extends Form
{
	function ListManageCommentForm()
	{
		Form::Form('ListManageCommentForm');
	...
	function save_position()
	{
		foreach($_REQUEST as $key=>$value)
		{
			...
			{
				DB::update_id('comment',array('position'=>Url::get('position_'.$match[1])),$match[1]);
			}
		}
	...
	function delete()
	{
		if(isset($_REQUEST['selected_ids']) and count($_REQUEST['selected_ids'])>0)
		{
			...
			{
				if($item = DB::exists_id('comment',$key))
				{
					save_recycle_bin('comment',$item);
					DB::delete_id('comment',intval($key));
					save_log($key);
				}
			}
		}
	...
	function draw()
	{
		$cond = '1 and comment.type="COMMENT"';
		...
		$total = ManageCommentDB::get_total_item($cond);
		$paging = paging($total,$item_per_page,10,false,'page_no',array('cmd','type','category_id'));
		$items = ManageCommentDB::get_items($cond,'comment.id DESC',$item_per_page);
		...
		$this->parse_layout('list',$this->just_edited_id+array(
			...
			'author_list'=>array(Portal::language('select_user'))+String::get_list(ManageCommentDB::get_user()),
	...
	function get_condition()
	{
		...
		if(Url::get('search'))
		{
			$cond .= URL::get('search')? ' AND ((comment.title) LIKE "%'.addslashes(URL::sget('search')).'%")':'';
		}
		return $cond;
	}
}
