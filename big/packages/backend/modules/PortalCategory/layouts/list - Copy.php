...
<fieldset id="toolbar">
	<div id="toolbar-title">
		Quản lý danh mục <span>[ <?php echo Portal::language(Url::get('cmd','list'));?> ]</span>
	</div>
	...
	<form name="ListCategoryForm" method="post">
		...
		<thead>
				...
				<th nowrap align="left">
					...
					<?php if(URL::get('order_by')=='category.name_'.Portal::language()) echo '<img alt="" src="assets/default/images/buttons/'.((URL::get('order_dir')!='desc')?'down':'up').'_arrow.gif">';?>
					Tên </a>				</th>
				<!--IF:cond1(Url::get('page')=='portal_category')--><th nowrap align="left"><a>[[.type.]]</a></th><!--/IF:cond1-->
				<th nowrap align="left">
					...
					<?php if(URL::get('order_by')=='category.status') echo '<img alt="" src="assets/default/images/buttons/'.((URL::get('order_dir')!='desc')?'down':'up').'_arrow.gif">';?>
					[[.status.]]					</a>				</th>
				<th align="left" nowrap="nowrap">Type</th>
				<th width="10%" align="left" nowrap="nowrap">Structure</th>
			...
			<tr bgcolor="<?php if((URL::get('just_edited_id',0)==[[=items.id=]]) or (is_numeric(array_search(MAP['items']['current']['id'],[[=just_edited_ids=]])))){ echo '#F7F7F7';} else {echo 'white';}?>" valign="middle" <?php Draw::hover(Portal::get_setting('crud_item_hover_bgcolor','#FFFFDD'));?> style="cursor:hand;<?php if([[=items.i=]]%2){echo 'background-color:#F9F9F9';}?>" id="Category_tr_[[|items.id|]]">
				...
				<td align="left" nowrap <?php if(User::can_edit(false,ANY_CATEGORY) and Url::get('page')!='portal_category'){?> <?php }?>><img src="[[|items.icon_url|]]" width="30" /></td>
				<td align="left" <?php if(User::can_edit(false,ANY_CATEGORY) and Url::get('page')!='portal_category'){?> <?php }?>>
						[[|items.indent|]]
						[[|items.indent_image|]]
						<span class="page_indent">&nbsp;</span>
						<?php if(User::can_edit(false,ANY_CATEGORY)){?><a href="<?php echo Url::build_current().'&cmd=edit&id='.[[=items.id=]];?>">[[[|items.id|]]] <?php }?><strong>[[|items.name|]]</strong></a><br><span style="color:#999;">Alias: [[|items.name_id|]]</span></td>
				<!--IF:cond1(Url::get('page')=='portal_category')--><td>[[|items.type|]]</td><!--/IF:cond1-->
				<td nowrap align="left">
						[[|items.status|]]					</td>
				<td align="left" nowrap="nowrap"> [[|items.type|]] </td>
				<td align="left" nowrap="nowrap"> [[|items.structure_id|]] </td>
				<?php if(User::can_edit(false,ANY_CATEGORY))
