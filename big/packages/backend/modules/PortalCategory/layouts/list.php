...
<fieldset id="toolbar">
    ...
    <div class="tree well">
        <div class="input-group">
            <label for="Category_all_checkbox" class="input-group-addon" id="basic-addon1">
                <input type="checkbox" value="1" id="Category_all_checkbox" onclick="select_all_checkbox(this.form,'Category',this.checked,'<?php echo Portal::get_setting('crud_list_item_selected_bgcolor','#FFFFEC');?>','<?php echo Portal::get_setting('crud_item_bgcolor','white');?>');"<?php if(URL::get('cmd')=='delete') echo ' checked';?>> Chọn tất cả</label>
            <select name="status" id="status" aria-describedby="basic-addon1"></select>
            <input type="button" value="OK" onClick="make_cmd('update_status');" class="btn btn-default">
        </div>
        [[|category|]]
    </div>
  </div>
