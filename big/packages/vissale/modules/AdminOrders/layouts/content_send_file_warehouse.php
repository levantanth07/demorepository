<?php
    require_once ROOT_PATH . 'packages/core/includes/utils/phpqrcode/qrlib.php';
    ...
    $ordersType = [];
    $getFullPhoneNumberPrintOrder = [[=getFullPhoneNumberPrintOrder=]];
                    ...
                    if ($kc != 'SP') {
                        if ($variable['name_db'] == $ki) {
                            if ($ki == 'bar_code' || $ki == 'postal_bar_code' || $ki == 'bar_code_id') {
                                ...
                                $results[$k_id]['nomalItems'][$kv] = '<div style="text-align: center; padding-top: 2px;font-size:11px;"><img src="data:image/png;base64,' . base64_encode($generator->getBarcode($val_i, $generator::TYPE_CODE_128)) . '"><br> <center>'. $val_i .'</center></div>';
                                ...
                                $results[$k_id]['nomalItems'][$kv] = '<div style="text-align: center; padding-top: 2px;font-size:11px;"><img src="data:image/png;base64,'. $result_qr_content_in_base64 .'"><br> <center>'. $val_i .'</center></div>';
                        ...
                        if ($variable['name_db'] == 'bar_code_large') {
                            $results[$k_id]['nomalItems']['{__MA_VACH_DH_SIZE_LARGE__}'] = '<div style="text-align: center; padding-top: 2px;font-size:11px;"><img src="data:image/png;base64,' . base64_encode($generator->getBarcode($items[$k_id]['bar_code'], $generator::TYPE_CODE_128)) . '&size=60"><br> <center>'. $items[$k_id]['bar_code'] .'</center></div>';
                        }
                        if ($variable['name_db'] == 'bar_code_large_qr') {
                            ob_start();
                            QRcode::png($items[$k_id]['bar_code']);
                            $result_qr_content_in_png = ob_get_contents();
                            ob_end_clean();
                            header("Content-type: text/html");
                            $result_qr_content_in_base64 =  base64_encode($result_qr_content_in_png);
                            $results[$k_id]['nomalItems']['{__MA_VACH_DH_SIZE_LARGE_QR__}'] = '<div style="text-align: center; padding-top: 2px;font-size:11px;"><img src="data:image/png;base64,'. $result_qr_content_in_base64 .'"><br> <center>'. $items[$k_id]['bar_code'] .'</center></div>';
                        }
                        ...
                        if ($variable['name_db'] == 'postal_bar_code_large') {
                            ...
                            $results[$k_id]['nomalItems']['{__MA_VACH_VAN_DON_SIZE_LARGE__}'] = '<div style="text-align: center; padding-top: 2px;font-size:11px;"><img src="data:image/png;base64,' . base64_encode($generator->getBarcode($items[$k_id]['postal_bar_code'], $generator::TYPE_CODE_128)) . '&size=60"><br> <center>'. $items[$k_id]['postal_bar_code'] .'</center></div>';
                        }
                        if ($variable['name_db'] == 'postal_bar_code_large_qr') {
                            ...
                            ob_start();
                            QRcode::png($items[$k_id]['postal_bar_code']);
                            $result_qr_content_in_png = ob_get_contents();
                            ob_end_clean();
                            header("Content-type: text/html");
                            $result_qr_content_in_base64 =  base64_encode($result_qr_content_in_png);
                            $results[$k_id]['nomalItems']['{__MA_VACH_VAN_DON_SIZE_LARGE_QR__}'] = '<div style="text-align: center; padding-top: 2px;font-size:11px;"><img src="data:image/png;base64,'. $result_qr_content_in_base64 .'"><br> <center>'. $items[$k_id]['postal_bar_code'] .'</center></div>';
                        }
                ...
                <div id="content">
                    <div class="box box-solid">
                        <div class="box-body" id="box-content">
                            <!--  -->
                            <style>
                                 b, p {font-family: DejaVu Sans, sans-serif !important;}
                            @media print {
                                #btn-print, .hidden-print, .navbar-fixed-bottom {
                                    display: none;
                                }
...
<script>
    ...
    $(function() {
        ...
        if (Object.keys(page_variables).length) {
            var j = 1;
            var item_per_page = 8;
            if (paper_size == 'A4IN8_BARCODE') {
                item_per_page = 6
            } else if (paper_size == 'A4IN4DOC' || paper_size == 'A4IN4') {
                item_per_page = 4
            } else if(paper_size == 'K80'){
                item_per_page = 1
            } else if(paper_size == 'A4IN8'){
                item_per_page = 8
            } else if (paper_size == 'A4-A5'){
                item_per_page = 1
            } else if (paper_size == 'A4-A5-NGANG') {
                item_per_page = 1
            } else if (item_per_page='A4IN6') {
                item_per_page = 6
            }
