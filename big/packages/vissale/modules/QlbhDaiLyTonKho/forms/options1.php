...
class QlbhDaiLyTonKhoOptionsForm extends Form
{
	...
	function get_store_card(){
		if(Url::get('code') and $row = DB::select('qlbh_product','code = \''.Url::get('code').'\'')){
			...
			$sql = '
				...
				FROM
					qlbh_inventory
				WHERE	
					qlbh_inventory.product_code = \''.$row['code'].'\' AND warehouse_id='.Session::get('warehouse_id').'
			';
			if($product = DB::fetch($sql)){
				$this->map['start_remain'] = $product['opening_stock'];	
			}else{
				$this->map['start_remain'] = 0;	
			}
