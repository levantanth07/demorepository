...
<?php
    $title = 'Báo cáo kho số Marketing';
    $items = [[=reports=]];
?>
<style>
                        ...
                        <table width="100%" class="table table-bordered tableFixHead" border="1" cellpadding="5" cellspacing="0">
                            <thead style="position: sticky; top: 0; z-index: 1">
                                <tr style="font-weight:bold;background:#5cbdf5;color:#fff;text-align: center;">
                                    ...
                                    <td>Họ và tên</td>
                                    <td>Số mới</td>
                                    <td>Số cũ <span title="Số CSKH, Tối ưu, ..." class="fa fa-question-circle"></span></td>
                                    <td>Tổng số cấp</td>
                                    <td>% Chốt <span title="% chốt = số chốt/số cấp" class="fa fa-question-circle"></span></td>
                            ...
                            <tbody>
                                ...
                                <?php foreach($items as $key => $value) : ?>
                                    <?php
                                        $total_so_moi += $value['so_moi'];
                                        $total_so_cu += ($value['so_cap'] - $value['so_moi']);
                                        $total_so_cap += $value['so_cap'];
                                        $total_so_chot += $value['so_chot'];
                                        $total_doanh_thu += $value['doanh_thu'];
                                        $total_so_ton += ($value['so_cap'] - $value['so_chia']);
                                    ...
                                    <tr class="text-center">
                                        ...
                                        <td><?php echo $value['so_moi'] ?></td>
                                        <td><?php echo ($value['so_cap'] - $value['so_moi']) ?></td>
                                        <td align="center">
                                            <div class="bar-wrapper" style="background: #efefef;">
                                                <div class="bar" style="background: #7fff00; width:<?=(([[=so_cap_max=]]>0)?(($value['so_cap']/[[=so_cap_max=]])*100):'0')?>%;">
                                                    <div style="z-index: 100;"><?php echo $value['so_cap'] ?  $value['so_cap'] :''?></div>
                                                </div>
                                            </div>
                                        </td>
                                        <td align="center">
                                            <div class="bar-wrapper" style="background: #efefef;width: 100%;">
                                                <div class="bar" style="background: <?php echo $value['ty_le_chot_color']; ?>;width: <?php echo $value['phan_tram_chot'] > 100 ? 100 : $value['phan_tram_chot'] ?>%;">
                                                    <div style="z-index: 100;">
                                                        <?php echo $value['phan_tram_chot'] ? $value['phan_tram_chot'].'%':'';?> 
                                                    </div>
                                                </div>
                                            </div>
                                        </td>
                                        <td align="center">
                                            <div class="bar-wrapper" style="background: #efefef;">
                                                <div class="bar" style="background: <?php echo $value['ty_le_ket_noi_color']; ?>;width: <?php echo $value['phan_tram_ket_noi'] > 100 ? 100 : $value['phan_tram_ket_noi'] ?>%;">
                                                    <div style="z-index: 100;">
                                                        <?php echo $value['phan_tram_ket_noi'] ? $value['phan_tram_ket_noi'].'%':'';?> 
                                                    </div>
                                                </div>
                                            </div>
                                        </td>
                                        <td><?php echo $value['so_chia'] ?></td>
                                        <td><?php echo $value['so_da_goi'] ?></td>
                                        <td><?php echo $value['so_huy'] ?></td>
                                        <td><?php echo $value['so_tiep_can'] ?></td>
                                        <td><?php echo $value['so_chot'] ?></td>
                                        <td><?php echo ($value['so_cap'] - $value['so_chia']) ?></td>
                            ...
                            <tfoot style="position: sticky; bottom: 0; background: #fff; font-weight: bold;">
                                <tr style="position: sticky; left: 0; background: #DDD;font-weight: bold" class="text-center">
                                    ...
                                    <td><?php echo $total_so_moi; ?></td>
                                    <td><?php echo $total_so_cu; ?></td>
                                    <td><?php echo $total_so_cap; ?></td>
                                    <td><?php echo $total_so_cap > 0 ? (round($total_so_chot/$total_so_cap,2)*100) : 0  ?> %</td>
