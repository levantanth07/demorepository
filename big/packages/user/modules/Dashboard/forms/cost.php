...
class CostForm extends Form
{
    ...
    public function __construct()
    {
        ...
        require_once ROOT_PATH . 'packages/user/modules/Dashboard/forms/Cost/FilterCondition.php';
    ...
    public function on_draw()
    {
        switch (URL::getString('act')) {
            ...
            case 'load_report':
                $statistics = [];
                if (FilterCondition::validateTimeRange()) {
        ...
        $this->map['date_from'] = date('m/d/Y', FilterCondition::getTimeFrom());
        $this->map['date_to'] = date('m/d/Y', FilterCondition::getTimeTo());
        
        $this->parse_layout('cost', $this->map);
    }
    ...
    {
        ['days' => $days, 'statistics' => $statistics] = $this->getDaysStatistics(URL::getArray('groups'));
    ...
    {
        $statistics = FilterCondition::validateTimeRange() ? $this->getGroupsStatistics(URL::getArray('groups')) : [];
...
">
    ...
    {
        $toDateTime = new DateTime(date('Y-m-d', FilterCondition::getTimeTo()));
        $fromDateTime = new DateTime(date('Y-m-d', FilterCondition::getTimeFrom()));
