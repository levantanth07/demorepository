...
class ReportForm extends Form
{
    ...
    function draw()
    {
        ...
        // $users = DashboardDB::get_users('MARKETING',Url::iget('is_active'),true);
        $userRequest = [];
        // $users = DashboardDB::getUserReportMktAndUpsale();
        $code = 'MARKETING';
        $users = DashboardDB::getUserNew($code, $this->selectIsActive);
        ...
        $this->map['users_id_list'] = [''=>'Tất cả MKT'] + MiString::get_list($users,'full_name');
        ...
        $this->parse_layout('doanh_thu_mkt',$this->map);
    }
    private function getDataConfirmed($condCfType, $start_time, $end_time, $strUserIds, $date_type)
    {
        $condCfType .= ' and orders.user_created IN ('.$strUserIds.')';
        $join = ' LEFT JOIN 
    ...
    private function getDataHuy($condHuy, $start_time, $end_time, $strUserIds, $date_type){
        ...
        $condHuy .= ' and orders.user_created IN ('.$strUserIds.') ';
        if($upsale_from_user_id=Url::get('upsale_from_user_id')){
            $condHuy .= ' AND orders_extra.upsale_from_user_id='. DB::escape($upsale_from_user_id); 
    ...
    private function getDataKeToan($condKeToan, $start_time, $end_time, $strUserIds, $date_type){
        ...
        $condKeToan .= ' and orders.user_created IN (' . $strUserIds . ')';
    ...
    private function getDataThanhCong($condThanhCong,$start_time, $end_time, $strUserIds, $date_type){
        ...
        $condThanhCong .= ' AND orders.user_created IN ('.$strUserIds.')';
        $sqlThanhCong = '
                        SELECT
                            orders.user_created as id,
    ...
    private function getDataDaThuTien($condDaThuTien, $start_time, $end_time, $strUserIds,$date_type){
        ...
        $condDaThuTien .= ' AND orders.user_created IN ('.$strUserIds.')';
    ...
    private function processDataMKT($users,$no_revenue_status,$start_time,$end_time,$account_type,$group_id,$master_group_id,$dataStatus,$tong_cong_ty,$date_type,$userRequest, $current_account_id, $strBundleRequets){
        ...
        if(Url::get('view_report')){
            ...
            $con_tiep_can = '
                    ...
                    and orders.user_created IN ('.$strUserIds.') and IFNULL(statuses.not_reach,0) <> 1
                    ';
            if($strBundleRequets){
