...
{   
    ...
    {   
        if(self::$timeFrom > self::$timeTo || self::$timeTo > time()){
            Form::set_flash_message(CostHkdForm::FLASH_MESSAGE_KEY, 'Thời gian bắt đầu và kết thúc không hợp lệ');
            return false;
        }

        if(self::$timeTo - self::$timeFrom > 31 * 86400){
            Form::set_flash_message(CostHkdForm::FLASH_MESSAGE_KEY, 'Vui lòng chọn khoảng thời gian tối đa là một tháng.');
            return false;
        }

        if(date('Ym', self::$timeTo) - date('Ym', self::$timeFrom)!== 0){
            Form::set_flash_message(CostHkdForm::FLASH_MESSAGE_KEY, 'Vui lòng chọn khoảng thời gian trong một tháng.');
            return false;
        }
